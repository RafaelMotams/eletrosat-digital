...
// Remover duplicatas de INEP (pegar a mais recente)
const seen = new Set();
const unique = [];
for (const r of rows) {
  if (!seen.has(r.inep)) {
...
writeFileSync('/tmp/ineps_data.json', JSON.stringify(unique));
console.log(`Exportados ${unique.length} registros únicos de ${rows.length} total`);

// Resumo por técnico
const byTecnico = {};
for (const r of unique) {
  const t = r.tecnico || 'Sem técnico';
  if (!byTecnico[t]) byTecnico[t] = { concluidas: 0, pendentes: 0, total: 0 };
  byTecnico[t].total++;
  if (r.status === 'concluida') byTecnico[t].concluidas++;
  else byTecnico[t].pendentes++;
}
console.log('\nResumo por técnico:');
for (const [t, v] of Object.entries(byTecnico)) {
  console.log(`  ${t}: ${v.concluidas} concluídas / ${v.pendentes} pendentes / ${v.total} total`);
}
