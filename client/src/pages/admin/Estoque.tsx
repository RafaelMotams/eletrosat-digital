...
import { Boxes, ClipboardCheck, ClipboardList, ArrowDownToLine, ArrowRightLeft, PackagePlus, Search, TriangleAlert, Undo2, Warehouse, Wrench, FileSpreadsheet } from "lucide-react";
...
function MovementDialog({
  title, description, materials, technicians, action, children, solicitacaoId,
}: {
  ...
  action: (data: { materialId: number; quantidade: number; tecnicoId?: number; observacao?: string; solicitacaoId?: number }) => Promise<void>; children?: React.ReactNode; solicitacaoId?: number;
}) {
  const [open, setOpen] = useState(false);
  ...
  async function submit() {
    ...
    setSaving(true);
    try {
      await action({ materialId: Number(materialId), quantidade: Number(quantidade), tecnicoId: tecnicoId ? Number(tecnicoId) : undefined, observacao: observacao.trim() || undefined, solicitacaoId });
...
function ReturnDialog({ saldo, onReturn }: { saldo: { materialId: number; nome: string; codigo: string; unidade: string; quantidade: string | number; holderId: number; tecnicoNome?: string | null }; onReturn: (data: { materialId: number; tecnicoId: number; quantidade: number; observacao: string }) => Promise<void> }) {
  const [open, setOpen] = useState(false);
  const [quantidade, setQuantidade] = useState("");
  const [observacao, setObservacao] = useState("");
  const [saving, setSaving] = useState(false);
  ...
  async function submit() {
    if (!quantidade || Number(quantidade) <= 0 || Number(quantidade) > disponivel) return toast.error("Informe uma quantidade válida dentro do saldo do técnico.");
    if (observacao.trim().length < 3) return toast.error("Informe o motivo da devolução para auditoria.");
    setSaving(true);
    try {
      await onReturn({ materialId: saldo.materialId, tecnicoId: saldo.holderId, quantidade: Number(quantidade), observacao: observacao.trim() });
      setOpen(false); setQuantidade(""); setObservacao("");
    } finally { setSaving(false); }
  }
  return <Dialog open={open} onOpenChange={setOpen}>
    <DialogTrigger asChild><Button variant="outline" size="sm" className="gap-2"><Undo2 className="h-4 w-4" /> Devolver</Button></DialogTrigger>
    <DialogContent className="sm:max-w-lg">
      <DialogHeader><DialogTitle>Devolver ao almoxarifado</DialogTitle><DialogDescription>{saldo.nome} está em posse de {saldo.tecnicoNome || `Técnico #${saldo.holderId}`}. A devolução retorna o material ao estoque da empresa e registra a auditoria.</DialogDescription></DialogHeader>
      <div className="grid gap-4 py-2">
        <p className="rounded-lg bg-muted px-3 py-2 text-xs text-muted-foreground">Disponível com o técnico: <strong>{formatQuantity(disponivel)} {saldo.unidade}</strong></p>
        <div className="grid gap-2"><Label>Quantidade devolvida</Label><Input type="number" min="0.001" max={disponivel} step="0.001" value={quantidade} onChange={event => setQuantidade(event.target.value)} /></div>
        <div className="grid gap-2"><Label>Motivo da devolução</Label><Input value={observacao} onChange={event => setObservacao(event.target.value)} placeholder="Ex.: material não utilizado na rota" /></div>
      </div>
      <DialogFooter><Button variant="outline" onClick={() => setOpen(false)}>Cancelar</Button><Button onClick={submit} disabled={saving}>{saving ? "Registrando..." : "Confirmar devolução"}</Button></DialogFooter>
    </DialogContent>
  </Dialog>;
}
...
export default function AdminEstoque() {
  ...
  const [historyInicio, setHistoryInicio] = useState("");
  const [historyFim, setHistoryFim] = useState("");
  ...
  const solicitacoesQuery = trpc.estoque.solicitacoes.listar.useQuery();
  const movimentosInput = useMemo(() => ({
    tecnicoId: historyTecnicoId === "todos" ? undefined : Number(historyTecnicoId),
    tipo: historyTipo === "todos" ? undefined : historyTipo as "entrada" | "transferencia" | "consumo" | "devolucao" | "ajuste",
    inicio: historyInicio ? new Date(`${historyInicio}T00:00:00`) : undefined,
    fim: historyFim ? new Date(`${historyFim}T00:00:00`) : undefined,
  }), [historyTecnicoId, historyTipo, historyInicio, historyFim]);
  const movimentosQuery = trpc.estoque.movimentacoes.list.useQuery(movimentosInput);
  ...
  const devolver = trpc.estoque.movimentacoes.devolver.useMutation({ onSuccess: async () => { await Promise.all([utils.estoque.saldos.list.invalidate(), utils.estoque.movimentacoes.list.invalidate()]); toast.success("Material devolvido ao almoxarifado."); }, onError: error => toast.error(error.message) });
  ...
  const atualizarSolicitacao = trpc.estoque.solicitacoes.atualizar.useMutation({ onSuccess: async () => { await solicitacoesQuery.refetch(); toast.success("Solicitação atualizada."); }, onError: error => toast.error(error.message) });
  ...
  const historyHasFilter = historyTecnicoId !== "todos" || historyTipo !== "todos" || Boolean(historyInicio) || Boolean(historyFim);
  const solicitacoes = solicitacoesQuery.data ?? [];
  const solicitacoesAbertas = solicitacoes.filter(s => s.status === "aberta" || s.status === "em_analise");
  const limparFiltrosHistorico = () => {
    setHistoryTecnicoId("todos");
  ...
  return <AdminLayoutTenant title="Estoque" subtitle="Controle materiais, almoxarifado e itens em posse dos técnicos" actions={<div className="flex flex-wrap gap-2"><Button variant="outline" className="gap-2" onClick={exportarEstoque} disabled={materials.length === 0}><FileSpreadsheet className="h-4 w-4" /> Exportar inventário</Button>{!isViewer && <><InventoryDialog materials={inventoryMaterials} onAdjust={async data => { await ajustar.mutateAsync(data); }} /><MovementDialog title="Registrar entrada" description="A entrada é adicionada ao almoxarifado desta empresa." materials={materials} action={async data => { await entrada.mutateAsync({ materialId: data.materialId, quantidade: data.quantidade, observacao: data.observacao }); }}><Button variant="outline" className="gap-2" disabled={actionDisabled}><ArrowDownToLine className="h-4 w-4" /> Registrar entrada</Button></MovementDialog><StockDialog onCreate={async data => { await createMaterial.mutateAsync(data); }} /></>}</div>}>
    ...
    <div className="grid gap-4 md:grid-cols-5 mb-6">
      ...
      <Card className={solicitacoesAbertas.length ? "border-indigo-200 bg-indigo-50/60" : ""}><CardContent className="pt-5"><p className="text-sm text-muted-foreground">Pedidos de reposição</p><p className="mt-1 text-3xl font-bold">{solicitacoesAbertas.length}</p><p className="mt-1 text-xs text-indigo-800">Abertos pelos técnicos</p></CardContent></Card>
    </div>
    ...
    <Tabs defaultValue="catalogo" className="space-y-5">
      <TabsList><TabsTrigger value="catalogo" className="gap-2"><Boxes className="h-4 w-4" /> Catálogo e almoxarifado</TabsTrigger><TabsTrigger value="tecnicos" className="gap-2"><Wrench className="h-4 w-4" /> Por técnico</TabsTrigger><TabsTrigger value="solicitacoes" className="gap-2"><ClipboardCheck className="h-4 w-4" /> Solicitações</TabsTrigger><TabsTrigger value="historico" className="gap-2"><ClipboardList className="h-4 w-4" /> Histórico</TabsTrigger></TabsList>
      ...
      <TabsContent value="tecnicos"><Card><CardHeader><CardTitle className="text-lg">Materiais em posse dos técnicos</CardTitle></CardHeader><CardContent className="p-0">{saldos.filter(s => s.holderType === "tecnico").length === 0 ? <OperationState kind="empty" title="Nenhum material transferido" description="Registre uma transferência do almoxarifado para acompanhar os itens de cada técnico." /> : <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-sm"><thead className="border-y bg-muted/40 text-left text-muted-foreground"><tr><th className="px-5 py-3 font-medium">Técnico</th><th className="px-4 py-3 font-medium">Material</th><th className="px-4 py-3 font-medium">Quantidade</th><th className="px-5 py-3 font-medium">Atualizado</th><th className="px-5 py-3 font-medium text-right">Ação</th></tr></thead><tbody>{saldos.filter(s => s.holderType === "tecnico").map(s => <tr key={s.saldoId} className="border-b last:border-0"><td className="px-5 py-4 font-medium">{s.tecnicoNome || `Técnico #${s.holderId}`}</td><td className="px-4 py-4"><p className="font-medium">{s.nome}</p><p className="text-xs text-muted-foreground">{s.codigo}</p></td><td className="px-4 py-4 font-semibold">{formatQuantity(s.quantidade)} {s.unidade}</td><td className="px-5 py-4 text-muted-foreground">—</td><td className="px-5 py-4 text-right">{!isViewer && <ReturnDialog saldo={s} onReturn={async data => { await devolver.mutateAsync(data); }} />}</td></tr>)}</tbody></table></div>}</CardContent></Card></TabsContent>
      <TabsContent value="solicitacoes"><Card><CardHeader><CardTitle className="text-lg">Solicitações de reposição</CardTitle><p className="text-sm text-muted-foreground">O pedido só é marcado como atendido após a transferência vinculada do almoxarifado, mantendo a entrega auditável.</p></CardHeader><CardContent className="p-0">{solicitacoesQuery.isLoading ? <OperationState kind="loading" title="Carregando solicitações" /> : solicitacoes.length === 0 ? <OperationState kind="empty" title="Nenhuma solicitação de reposição" description="Os pedidos enviados pelos técnicos aparecerão aqui para análise da empresa." /> : <div className="overflow-x-auto"><table className="w-full min-w-[860px] text-sm"><thead className="border-y bg-muted/40 text-left text-muted-foreground"><tr><th className="px-5 py-3 font-medium">Solicitado por</th><th className="px-4 py-3 font-medium">Material</th><th className="px-4 py-3 font-medium">Quantidade</th><th className="px-4 py-3 font-medium">Motivo</th><th className="px-4 py-3 font-medium">Status</th><th className="px-5 py-3 font-medium text-right">Ação</th></tr></thead><tbody>{solicitacoes.map(s => { const statusLabel = s.status === "aberta" ? "Aberta" : s.status === "em_analise" ? "Em análise" : s.status === "atendida" ? "Atendida" : "Cancelada"; const pendente = s.status === "aberta" || s.status === "em_analise"; return <tr key={s.id} className="border-b last:border-0"><td className="px-5 py-4"><p className="font-medium">{s.tecnicoNome}</p><p className="text-xs text-muted-foreground">{formatDate(s.createdAt)}</p></td><td className="px-4 py-4"><p className="font-medium">{s.materialNome}</p><p className="text-xs text-muted-foreground">{s.materialCodigo}</p></td><td className="px-4 py-4 font-semibold">{formatQuantity(s.quantidadeSolicitada)} {s.unidade}</td><td className="px-4 py-4 text-muted-foreground">{s.observacao || "—"}</td><td className="px-4 py-4"><span className="rounded-full bg-muted px-2 py-1 text-xs font-semibold">{statusLabel}</span>{s.resposta && <p className="mt-1 max-w-48 text-xs text-muted-foreground">{s.resposta}</p>}{s.atendimentoMovimentacaoId && <p className="mt-1 text-xs text-emerald-700">Transferência #{s.atendimentoMovimentacaoId}</p>}</td><td className="px-5 py-4 text-right">{!isViewer && pendente && <div className="flex justify-end gap-2"><Button variant="outline" size="sm" disabled={atualizarSolicitacao.isPending} onClick={() => atualizarSolicitacao.mutate({ id: s.id, status: "em_analise" })}>Analisar</Button><MovementDialog title="Atender solicitação" description="A transferência entrega o material ao técnico e marca este pedido como atendido." materials={[{ id: s.materialId, nome: s.materialNome, codigo: s.materialCodigo }]} technicians={[{ id: s.tecnicoId, nome: s.tecnicoNome }]} solicitacaoId={s.id} action={async data => { await transferir.mutateAsync({ materialId: data.materialId, tecnicoId: data.tecnicoId!, quantidade: data.quantidade, observacao: data.observacao, solicitacaoId: data.solicitacaoId }); }}><Button size="sm">Atender</Button></MovementDialog></div>}</td></tr>; })}</tbody></table></div>}</CardContent></Card></TabsContent>
      <TabsContent value="historico"><Card><CardHeader className="space-y-4"><div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><CardTitle className="text-lg">Movimentações auditáveis</CardTitle>{historyHasFilter && <Button type="button" variant="ghost" size="sm" onClick={limparFiltrosHistorico}>Limpar filtros</Button>}</div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Select value={historyTecnicoId} onValueChange={setHistoryTecnicoId}><SelectTrigger><SelectValue placeholder="Todos os técnicos" /></SelectTrigger><SelectContent><SelectItem value="todos">Todos os técnicos</SelectItem>{technicians.map(tecnico => <SelectItem key={tecnico.id} value={String(tecnico.id)}>{tecnico.nome}</SelectItem>)}</SelectContent></Select><Select value={historyTipo} onValueChange={setHistoryTipo}><SelectTrigger><SelectValue placeholder="Todos os movimentos" /></SelectTrigger><SelectContent><SelectItem value="todos">Todos os movimentos</SelectItem><SelectItem value="entrada">Entrada</SelectItem><SelectItem value="transferencia">Transferência</SelectItem><SelectItem value="consumo">Consumo</SelectItem><SelectItem value="devolucao">Devolução</SelectItem><SelectItem value="ajuste">Ajuste</SelectItem></SelectContent></Select><div className="grid gap-1"><Label className="text-xs text-muted-foreground">A partir de</Label><Input type="date" value={historyInicio} onChange={event => setHistoryInicio(event.target.value)} /></div><div className="grid gap-1"><Label className="text-xs text-muted-foreground">Até</Label><Input type="date" value={historyFim} onChange={event => setHistoryFim(event.target.value)} /></div></div></CardHeader><CardContent className="p-0">{movimentosQuery.isLoading ? <OperationState kind="loading" title="Carregando movimentações" /> : (movimentosQuery.data?.length ?? 0) === 0 ? <OperationState kind="empty" title={historyHasFilter ? "Nenhuma movimentação encontrada" : "Nenhuma movimentação registrada"} description={historyHasFilter ? "Ajuste ou limpe os filtros para consultar outro período." : "Entradas, transferências, consumos, devoluções e ajustes aparecerão aqui com rastreabilidade."} actionLabel={historyHasFilter ? "Limpar filtros" : undefined} onAction={historyHasFilter ? limparFiltrosHistorico : undefined} /> : <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-sm"><thead className="border-y bg-muted/40 text-left text-muted-foreground"><tr><th className="px-5 py-3 font-medium">Data</th><th className="px-4 py-3 font-medium">Material</th><th className="px-4 py-3 font-medium">Tipo</th><th className="px-4 py-3 font-medium">Quantidade</th><th className="px-5 py-3 font-medium">Referência</th></tr></thead><tbody>{movimentosQuery.data?.map(m => <tr key={m.id} className="border-b last:border-0"><td className="px-5 py-4 text-muted-foreground">{formatDate(m.createdAt)}</td><td className="px-4 py-4"><p className="font-medium">{m.materialNome}</p><p className="text-xs text-muted-foreground">{m.materialCodigo}</p></td><td className="px-4 py-4 capitalize">{m.tipo}</td><td className="px-4 py-4 font-semibold">{formatQuantity(m.quantidade)}</td><td className="px-5 py-4 text-muted-foreground">{m.ordemServicoId ? `OS #${m.ordemServicoId}` : m.manutencaoId ? `Manutenção #${m.manutencaoId}` : m.observacao || "—"}</td></tr>)}</tbody></table></div>}</CardContent></Card></TabsContent>
    </Tabs>
  </AdminLayoutTenant>;
}
