...
export default function TecnicoEstoque() {
  ...
  const filtered = useMemo(() => items.filter(item => `${item.codigo} ${item.nome} ${item.categoria ?? ""}`.toLowerCase().includes(search.toLowerCase())), [items, search]);
  ...
  return <div className="min-h-screen pb-28" style={{ background: "linear-gradient(160deg, #020817 0%, #071328 48%, #020817 100%)" }}>
    ...
    <main className="mx-auto max-w-2xl px-4 py-5 space-y-4">
      ...
      <section className="grid grid-cols-2 gap-3">
        ...
        <div className="rounded-2xl p-4" style={{ background: lowCount ? "linear-gradient(135deg, rgba(245,158,11,.14), rgba(239,68,68,.06))" : "rgba(255,255,255,.035)", border: `1px solid ${lowCount ? "rgba(245,158,11,.24)" : "rgba(255,255,255,.07)"}` }}><AlertTriangle className={`h-5 w-5 ${lowCount ? "text-amber-300" : "text-slate-500"}`} /><p className="mt-3 text-2xl font-black text-white">{lowCount}</p><p className="text-xs font-medium text-slate-400">Itens para reposição</p></div>
      </section>
