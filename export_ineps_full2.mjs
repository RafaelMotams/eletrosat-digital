...
// Usa COALESCE para pegar técnico da OS se existir, senão usa técnico da escola
const [rows] = await conn.execute(`
  SELECT 
    ...
    COALESCE(t_os.nome, t_esc.nome, 'Sem técnico') AS tecnico,
  ...
  LEFT JOIN tecnicos t_os ON t_os.id = os.tecnicoId
  LEFT JOIN tecnicos t_esc ON t_esc.id = e.tecnicoId
  WHERE e.inep IN (${placeholders})
  ORDER BY COALESCE(t_os.nome, t_esc.nome) ASC, e.municipio ASC, e.nome ASC
`, ineps);
await conn.end();

// Remover duplicatas de INEP (manter a mais recente/com OS)
const seen = new Set();
const unique = [];
for (const r of rows) {
  if (!seen.has(r.inep)) {
...
writeFileSync('/tmp/ineps_full2.json', JSON.stringify(unique));
console.log(`Exportados ${unique.length} registros únicos`);

// Resumo por técnico
const byTec = {};
for (const r of unique) {
  const t = (r.tecnico || 'Sem técnico').trim();
  if (!byTec[t]) byTec[t] = 0;
  byTec[t]++;
}
console.log('\nResumo por técnico:');
for (const [t, qtd] of Object.entries(byTec).sort((a,b) => b[1]-a[1])) {
  console.log(`  ${t}: ${qtd} escolas`);
}
